...
const fetchBlogs = async () => {
    // Fetch posts from 'blog-posts' collection, filter by category 'Music' and status 'published'
    ...
    const blogsSnapshot = await getDocs(musicQuery);

    if (blogsSnapshot.empty) {
        if(blogSection) blogSection.innerHTML = '<p class="no-posts">No music articles found.</p>';
        return;
    }
