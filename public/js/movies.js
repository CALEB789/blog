...
const fetchBlogs = async () => {
    // Fetch posts from 'blog-posts' collection, filter by category 'Movies' and status 'published'
    ...
    const blogsSnapshot = await getDocs(moviesQuery);

    if (blogsSnapshot.empty) {
        if(blogSection) blogSection.innerHTML = '<p class="no-posts">No movie articles found.</p>';
        return;
    }
