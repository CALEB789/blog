...
const fetchBlogs = async () => {
    // Fetch posts from 'blog-posts' collection, filter by category 'News' and status 'published'
    ...
    const blogsSnapshot = await getDocs(newsQuery);
    
    if (blogsSnapshot.empty) {
        if(blogSection) blogSection.innerHTML = '<p class="no-posts">No news articles found.</p>';
        return;
    }
